...
class RamcharitmanasAyodhyakandaOpening11Tests(unittest.TestCase):
    ...
    def test_invocation_and_groups_are_contiguous(self) -> None:
        ...
        self.assertIsNone(self.pack["passages"][-1]["next_passage_id"])
    ...
    def test_contract_counts_and_forward_endpoint_match(self) -> None:
        ...
        self.assertEqual(len(progress["completed_contiguous_batches"]), 2)
        newest = progress["completed_contiguous_batches"][-1]
        self.assertEqual(newest["canonical_group_start"], "invocation")
        self.assertEqual(newest["canonical_group_end"], 11)
        self.assertFalse(newest["complete_division"])
    ...
    def test_inventories_match_the_reviewed_selected_scope(self) -> None:
        ...
        self.assertEqual(mvp_lane["current"]["consumer_complete_contiguous_passages"], 81)
        self.assertEqual(mvp_lane["current"]["consumer_complete_source_units"], 436)
        self.assertEqual(mvp_lane["current"]["contiguous_forward_endpoint"], "Ayodhyakanda doha 11")
        self.assertEqual(coverage_lane["denominator"]["completed_contiguous_passages"], 81)
        self.assertEqual(coverage_lane["denominator"]["completed_source_units"], 436)
        self.assertEqual(coverage_lane["denominator"]["contiguous_forward_endpoint"], "Ayodhyakanda doha 11")
