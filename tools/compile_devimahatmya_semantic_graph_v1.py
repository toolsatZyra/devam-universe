...
MIGRATION_PATH = ROOT / "supabase/migrations/20260808173000_add_devimahatmya_semantic_graph.sql"
...
def load_pack() -> dict[str, Any]:
    ...
    if len(pack.get("entities", [])) != 4 or len(pack.get("claims", [])) != 4:
    ...
    for claim in pack["claims"]:
        ...
        expected = (
            evidence["chapter"], evidence["verse"], SOURCE_SHA256, evidence["source_ordinal"], evidence["source_span_sha256"]
        )
    ...
    for claim in pack["claims"]:
        ...
        for language_code in ("en", "hi"):
            ...
            claim_rows.append([
                ...
                sql_quote(language_code), str(claim["_confidence"]), str(evidence["citation_ordinal"]), str(evidence["chapter"]),
...
begin
  ...
  if (select count(*) from public.source_objects where sha256={sql_quote(SOURCE_SHA256)} and rights_lane='derivative_allowed') <> 1 then
    raise exception 'Pinned Devimahatmya source object is absent, duplicated, or rights-incompatible';
  end if;
  if (select count(*) from public.passages where source_object_id=(select id from public.source_objects where sha256={sql_quote(SOURCE_SHA256)}) and source_ordinal in (68, 189, 229) and publication_state='published' and rights_lane='derivative_allowed') <> 3 then
...
select i.stable_key, subject.id, i.statement, i.language_code, 'source_bounded_narrative_index', 'devam_synthesis', i.confidence,
  jsonb_build_object('pack_id', {sql_quote(pack['pack_id'])}, 'pack_sha256', {sql_quote(pack_hash)}, 'work_slug', {sql_quote(boundary['work_slug'])},
    ...
    'citation_ordinal', i.citation_ordinal, 'chapter', i.chapter, 'verse', i.verse, 'scope', {sql_quote(boundary['scope'])},
...
select c.id, p.id, 'supports', jsonb_build_object(
  ...
  'source_sha256', {sql_quote(SOURCE_SHA256)}, 'source_ordinal', i.source_ordinal, 'source_span_sha256', i.source_span_sha256,
...
do $$
begin
  if (select count(*) from public.entities where slug=any(array[{entity_array}]) and rights_lane='derivative_allowed' and publication_state='published') <> 4 then raise exception 'Devimahatmya semantic entity verification failed'; end if;
  if (select count(*) from public.entity_names n join public.entities e on e.id=n.entity_id where e.slug=any(array[{entity_array}]) and n.is_preferred) <> 12 then raise exception 'Devimahatmya semantic preferred-name verification failed'; end if;
  if (select count(*) from public.claims where stable_key like {sql_quote(prefix + '%')} and claim_kind='source_bounded_narrative_index' and evidence_class='devam_synthesis' and rights_lane='derivative_allowed' and publication_state='published') <> 8 then raise exception 'Devimahatmya semantic claim verification failed'; end if;
  if (select count(*) from public.claim_evidence ce join public.claims c on c.id=ce.claim_id where c.stable_key like {sql_quote(prefix + '%')} and ce.evidence_role='supports') <> 8 then raise exception 'Devimahatmya semantic evidence verification failed'; end if;
  if (select count(*) from public.relationships r join public.entities s on s.id=r.subject_entity_id join public.entities o on o.id=r.object_entity_id join public.claims c on c.id=r.claim_id where s.slug={sql_quote(subject_slug)} and o.slug=any(array[{entity_array}]) and r.predicate='contains_narrative_of' and c.stable_key like {sql_quote(prefix + '%')} and r.rights_lane='derivative_allowed' and r.publication_state='published') <> 4 then raise exception 'Devimahatmya semantic relationship verification failed'; end if;
end $$;
...
def main() -> int:
    ...
    pack = load_pack()
    print(compile_sql(pack) if args.format == "sql" else json.dumps(build_report(pack), ensure_ascii=False, indent=2, sort_keys=True), end="" if args.format == "sql" else "\n")
    return 0
