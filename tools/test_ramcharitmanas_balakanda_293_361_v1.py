...
from tools.validate_ramcharitmanas_balakanda_293_361_v1 import (
    EXPECTED_NEW_PAGE_MAP,
    expected_kinds_for_group,
    load_pack,
    validate_pack,
)
...
class RamcharitmanasBalakanda293To361Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_interval_is_exactly_sixty_nine_contiguous_passages_and_377_units(self):
        passages = self.pack["passages"]
        self.assertEqual([int(row["canonical_group_label"]) for row in passages], list(range(293, 362)))
        self.assertEqual(len(passages), 69)
        self.assertEqual(sum(row["source_unit_count"] for row in passages), 377)
        units = [unit for passage in passages for unit in passage["source_units"]]
        self.assertEqual([unit["batch_unit_ordinal"] for unit in units], list(range(1, 378)))
        self.assertEqual([unit["source_order_key"] for unit in units], sorted(unit["source_order_key"] for unit in units))
        self.assertEqual(len({unit["unit_id"] for unit in units}), 377)
    ...
    def test_every_passage_has_fresh_approachable_hindi_and_english_meaning(self):
        ...
        self.assertTrue(self.pack["selected_scope"]["reaches_division_end"])
        self.assertIn("groups 1-292", self.pack["selected_scope"]["boundary"])
    ...
    def test_new_interval_keeps_devotional_poetic_and_social_claims_bounded(self):
        ...
        self.assertIn("not a modern measure of generosity", meanings[340]["en"])
        self.assertIn("not independent proof", meanings[341]["en"])
        self.assertIn("not a verified municipal record", meanings[344]["en"])
        self.assertIn("not a mandatory shopping list", meanings[346]["en"])
        self.assertIn("not weather observations", meanings[347]["en"])
        self.assertIn("disregard for a person's privacy", meanings[348]["en"])
        self.assertIn("not medical claims, body standards, proof of consent", meanings[350]["en"])
        self.assertIn("not universal duties", meanings[351]["en"])
        self.assertIn("not compulsory modern etiquette", meanings[352]["en"])
        self.assertIn("not turn people into property", meanings[353]["en"])
        self.assertIn("not be generalized into a rule", meanings[355]["en"])
        self.assertIn("not a verified battle report", meanings[356]["en"])
        self.assertIn("not permission for humiliation or harassment", meanings[358]["en"])
        self.assertIn("not polling data", meanings[359]["en"])
        self.assertIn("not evidence that refusal or coercion is acceptable", meanings[360]["en"])
        self.assertIn("devotional fruit claims, not guaranteed outcomes", meanings[361]["en"])
    ...
    def test_scan_coordinates_and_reference_only_boundary_are_explicit(self):
        ...
        self.assertEqual(pages, set(range(354, 425)))
        for passage in self.pack["passages"]:
    ...
    def test_new_passages_keep_the_exact_visual_scan_map_and_closing_labels(self):
        by_group = {int(row["canonical_group_label"]): row for row in self.pack["passages"]}
        for group, pages in EXPECTED_NEW_PAGE_MAP.items():
            self.assertEqual(by_group[group]["source_locator"]["scan_pages"], pages)
        self.assertEqual(
            [row["unit_label"] for row in by_group[350]["source_units"][-2:]],
            ["350a", "350b"],
        )
        self.assertEqual(
            [row["unit_label"] for row in by_group[361]["source_units"][-2:]],
            ["closing-chhand", "361"],
        )
    ...
    def test_reading_contract_counts_reviewed_batches_without_counting_old_commentary(self):
        ...
        self.assertEqual(progress["completed_passages"], 295)
        self.assertEqual(progress["completed_source_units"], 1516)
    ...
    def test_corpus_payload_is_not_duplicated_into_a_seed_migration_or_validator(self):
        ...
        validator = ROOT / "tools/validate_ramcharitmanas_balakanda_293_361_v1.py"
    ...
    def test_utf8_is_valid_without_common_mojibake(self):
        ...
        self.assertIn("क्रोध मोह ममता मद त्यागी", text)
        self.assertNotIn("कोहु मोहु ममता मदु त्यागी", text)
        self.assertIn("मनहुँ बलाक अवलि मनु करषहिं", text)
        self.assertNotIn("मनहुँबलाक अवलि मनु करषहिं", text)
