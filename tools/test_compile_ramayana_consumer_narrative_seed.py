...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 45 playable Ramayana turns", sql)
        self.assertIn("Expected 4 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 4 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 357 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1605 Ramayana narrative beats", sql)
        self.assertIn("Expected 812 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 3210 bilingual Ramayana beat texts", sql)
