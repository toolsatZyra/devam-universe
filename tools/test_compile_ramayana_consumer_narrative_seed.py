...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 17 playable Ramayana turns", sql)
        self.assertIn("Expected 32 unfinished Ramayana turns", sql)
        self.assertIn("Expected 3 outlined Ramayana turns", sql)
        self.assertIn("Expected 29 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 77 Ramayana playable scenes", sql)
        self.assertIn("Expected 20 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 375 Ramayana narrative beats", sql)
        self.assertIn("Expected 292 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 750 bilingual Ramayana beat texts", sql)
