...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 18 playable Ramayana turns", sql)
        self.assertIn("Expected 31 unfinished Ramayana turns", sql)
        self.assertIn("Expected 2 outlined Ramayana turns", sql)
        self.assertIn("Expected 29 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 84 Ramayana playable scenes", sql)
        self.assertIn("Expected 13 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 405 Ramayana narrative beats", sql)
        self.assertIn("Expected 292 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 810 bilingual Ramayana beat texts", sql)
