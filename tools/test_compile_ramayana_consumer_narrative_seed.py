...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 33 playable Ramayana turns", sql)
        self.assertIn("Expected 16 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 16 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 215 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1021 Ramayana narrative beats", sql)
        self.assertIn("Expected 528 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 2042 bilingual Ramayana beat texts", sql)
