...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 423 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1842 Ramayana narrative beats", sql)
        self.assertIn("Expected 944 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 3684 bilingual Ramayana beat texts", sql)
