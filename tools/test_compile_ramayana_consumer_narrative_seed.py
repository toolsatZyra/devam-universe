...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 25 playable Ramayana turns", sql)
        self.assertIn("Expected 24 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 24 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 135 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 618 Ramayana narrative beats", sql)
        self.assertIn("Expected 368 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1236 bilingual Ramayana beat texts", sql)
