...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 39 playable Ramayana turns", sql)
        self.assertIn("Expected 10 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 10 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 285 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1348 Ramayana narrative beats", sql)
        self.assertIn("Expected 668 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 2696 bilingual Ramayana beat texts", sql)
