...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 41 playable Ramayana turns", sql)
        self.assertIn("Expected 8 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 8 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 313 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1460 Ramayana narrative beats", sql)
        self.assertIn("Expected 724 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 2920 bilingual Ramayana beat texts", sql)
