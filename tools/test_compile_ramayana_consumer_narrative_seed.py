...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 37 playable Ramayana turns", sql)
        self.assertIn("Expected 12 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 12 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 257 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1234 Ramayana narrative beats", sql)
        self.assertIn("Expected 612 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 2468 bilingual Ramayana beat texts", sql)
