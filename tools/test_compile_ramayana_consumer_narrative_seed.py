...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 20 playable Ramayana turns", sql)
        self.assertIn("Expected 29 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 29 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 97 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 466 Ramayana narrative beats", sql)
        self.assertIn("Expected 292 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 932 bilingual Ramayana beat texts", sql)
