...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 48 playable Ramayana turns", sql)
        self.assertIn("Expected 1 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 1 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 389 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1707 Ramayana narrative beats", sql)
        self.assertIn("Expected 876 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 3414 bilingual Ramayana beat texts", sql)
