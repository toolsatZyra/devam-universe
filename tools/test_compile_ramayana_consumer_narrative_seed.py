...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 32 playable Ramayana turns", sql)
        self.assertIn("Expected 17 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 17 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 205 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 958 Ramayana narrative beats", sql)
        self.assertIn("Expected 508 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1916 bilingual Ramayana beat texts", sql)
