...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 26 playable Ramayana turns", sql)
        self.assertIn("Expected 23 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 23 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 148 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 670 Ramayana narrative beats", sql)
        self.assertIn("Expected 394 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1340 bilingual Ramayana beat texts", sql)
