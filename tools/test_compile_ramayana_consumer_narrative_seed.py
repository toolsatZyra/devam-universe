...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 31 playable Ramayana turns", sql)
        self.assertIn("Expected 18 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 18 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 190 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 870 Ramayana narrative beats", sql)
        self.assertIn("Expected 478 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1740 bilingual Ramayana beat texts", sql)
