...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 35 playable Ramayana turns", sql)
        self.assertIn("Expected 14 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 14 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 236 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1120 Ramayana narrative beats", sql)
        self.assertIn("Expected 570 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 2240 bilingual Ramayana beat texts", sql)
