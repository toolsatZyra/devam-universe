...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 30 playable Ramayana turns", sql)
        self.assertIn("Expected 19 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 19 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 180 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 816 Ramayana narrative beats", sql)
        self.assertIn("Expected 458 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1632 bilingual Ramayana beat texts", sql)
