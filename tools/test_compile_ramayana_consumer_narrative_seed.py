...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 27 playable Ramayana turns", sql)
        self.assertIn("Expected 22 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 22 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 158 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 710 Ramayana narrative beats", sql)
        self.assertIn("Expected 414 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 1420 bilingual Ramayana beat texts", sql)
