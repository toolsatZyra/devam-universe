...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 43 playable Ramayana turns", sql)
        self.assertIn("Expected 6 unfinished Ramayana turns", sql)
        self.assertIn("Expected 0 outlined Ramayana turns", sql)
        self.assertIn("Expected 6 orientation-only Ramayana turns", sql)
        self.assertIn("Expected 341 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 1557 Ramayana narrative beats", sql)
        self.assertIn("Expected 780 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 3114 bilingual Ramayana beat texts", sql)
