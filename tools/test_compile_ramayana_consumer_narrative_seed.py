...
class RamayanaConsumerNarrativeSeedTest(unittest.TestCase):
    ...
    def test_migration_keeps_outlines_draft_and_playable_scenes_source_addressed(self) -> None:
        ...
        self.assertIn("Expected 494 Ramayana playable scenes", sql)
        self.assertIn("Expected 0 draft Ramayana scene outlines", sql)
        self.assertIn("Expected 2211 Ramayana narrative beats", sql)
        self.assertIn("Expected 1086 bilingual Ramayana moment texts", sql)
        self.assertIn("Expected 4422 bilingual Ramayana beat texts", sql)
