...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 31
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 18) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 190
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 870) {
  ...
  if (new Set(momentSlugs).size !== 239) throw new Error("Narrative moment slugs must be unique");
