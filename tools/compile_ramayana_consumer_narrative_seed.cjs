...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 478
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 2122) {
  ...
  if (new Set(momentSlugs).size !== 527) throw new Error("Narrative moment slugs must be unique");
