...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 23
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 26) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 114
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 534) {
  ...
  if (new Set(momentSlugs).size !== 163) throw new Error("Narrative moment slugs must be unique");
