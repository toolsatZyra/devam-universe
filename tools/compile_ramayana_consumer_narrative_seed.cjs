...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 503
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 2259) {
  ...
  if (new Set(momentSlugs).size !== 552) throw new Error("Narrative moment slugs must be unique");
