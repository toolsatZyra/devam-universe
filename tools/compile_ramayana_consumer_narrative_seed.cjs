...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 47
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 2) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 381
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1677) {
  ...
  if (new Set(momentSlugs).size !== 430) throw new Error("Narrative moment slugs must be unique");
