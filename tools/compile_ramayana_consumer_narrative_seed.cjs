...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 43
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 6) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 341
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1557) {
  ...
  if (new Set(momentSlugs).size !== 390) throw new Error("Narrative moment slugs must be unique");
