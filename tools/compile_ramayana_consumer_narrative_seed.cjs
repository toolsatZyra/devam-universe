...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 423
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1842) {
  ...
  if (new Set(momentSlugs).size !== 472) throw new Error("Narrative moment slugs must be unique");
