...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 30
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 19) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 180
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 816) {
  ...
  if (new Set(momentSlugs).size !== 229) throw new Error("Narrative moment slugs must be unique");
