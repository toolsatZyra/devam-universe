...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 407
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1771) {
  ...
  if (new Set(momentSlugs).size !== 456) throw new Error("Narrative moment slugs must be unique");
