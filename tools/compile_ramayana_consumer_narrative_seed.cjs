...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 413
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1794) {
  ...
  if (new Set(momentSlugs).size !== 462) throw new Error("Narrative moment slugs must be unique");
