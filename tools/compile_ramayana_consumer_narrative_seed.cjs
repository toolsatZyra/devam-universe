...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 450
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1969) {
  ...
  if (new Set(momentSlugs).size !== 499) throw new Error("Narrative moment slugs must be unique");
