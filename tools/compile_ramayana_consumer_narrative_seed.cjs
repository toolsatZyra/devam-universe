...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 44
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 5) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 349
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1581) {
  ...
  if (new Set(momentSlugs).size !== 398) throw new Error("Narrative moment slugs must be unique");
