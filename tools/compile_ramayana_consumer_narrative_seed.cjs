...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 438
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1913) {
  ...
  if (new Set(momentSlugs).size !== 487) throw new Error("Narrative moment slugs must be unique");
