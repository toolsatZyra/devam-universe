...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 448
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1957) {
  ...
  if (new Set(momentSlugs).size !== 497) throw new Error("Narrative moment slugs must be unique");
