...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 29
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 20) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 176
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 788) {
  ...
  if (new Set(momentSlugs).size !== 225) throw new Error("Narrative moment slugs must be unique");
