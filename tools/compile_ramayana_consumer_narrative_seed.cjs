...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 452
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1981) {
  ...
  if (new Set(momentSlugs).size !== 501) throw new Error("Narrative moment slugs must be unique");
