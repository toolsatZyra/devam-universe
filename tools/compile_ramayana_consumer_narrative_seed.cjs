...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 42
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 7) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 326
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 1512) {
  ...
  if (new Set(momentSlugs).size !== 375) throw new Error("Narrative moment slugs must be unique");
