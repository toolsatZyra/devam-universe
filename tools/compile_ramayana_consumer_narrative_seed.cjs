...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableTurns !== 22
    || snapshot.counters.outlinedTurns !== 0
    || snapshot.counters.orientationOnlyTurns !== 27) {
    throw new Error("Unexpected Ramayana playable-turn boundary");
  }
  if (snapshot.counters.playableScenes !== 109
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 514) {
  ...
  if (new Set(momentSlugs).size !== 158) throw new Error("Narrative moment slugs must be unique");
