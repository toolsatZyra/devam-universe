...
function validateSnapshot(snapshot) {
  ...
  if (snapshot.counters.playableScenes !== 482
    || snapshot.counters.draftSceneOutlines !== 0
    || snapshot.counters.bilingualBeats !== 2141) {
  ...
  if (new Set(momentSlugs).size !== 531) throw new Error("Narrative moment slugs must be unique");
