...
import { RAMAYANA_SITA_BOW_PLAYABLE_SCENES } from "./ramayana-sita-bow-playable";

describe("Ramayana Sita-and-the-bow playable stories", () => {
  const outlineById = new Map(RAMAYANA_BEGINNINGS_SCENE_OUTLINES.map((outline) => [outline.id, outline]));

  it("completes seven source-partitioned scenes across Balakanda 49-65", () => {
    expect(RAMAYANA_SITA_BOW_PLAYABLE_SCENES).toHaveLength(7);
    expect(RAMAYANA_SITA_BOW_PLAYABLE_SCENES.reduce((count, scene) => count + scene.moment.beats.length, 0)).toBe(33);
    const covered = RAMAYANA_SITA_BOW_PLAYABLE_SCENES.flatMap((scene) => {
    ...
    expect(covered).toEqual(Array.from({ length: 17 }, (_, index) => index + 49));
  });

  it("keeps every scene substantial, bilingual, visual, and traversable", () => {
    for (const scene of RAMAYANA_SITA_BOW_PLAYABLE_SCENES) {
