...
describe("Sarthi forgiveness guidance pilot supplement", () => {
  const supplement = json("apps/web/evaluation/sarthi-forgiveness-pilot-supplement-v0.1.json") as {
    contract: string;
    status: string;
  ...
  it("binds the frozen pilot, research manifests and source-bounded evidence pack", () => {
    expect(supplement.contract).toBe("DEVAM_SARTHI_FORGIVENESS_GUIDANCE_PILOT_SUPPLEMENT_V0_1");
  ...
  it("changes mode only after continuing harm and renewed access are established", async () => {
    expect(pair?.variants).toHaveLength(2);
    if (!pair) throw new Error("moral-03 pair missing");
