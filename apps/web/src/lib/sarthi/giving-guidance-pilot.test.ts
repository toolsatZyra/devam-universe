...
describe("Sarthi proportional-giving pilot supplement", () => {
  const supplement = json("apps/web/evaluation/sarthi-giving-pilot-supplement-v0.1.json") as {
    contract: string;
    status: string;
  ...
  it("binds the frozen pilot, research manifests and source-bounded evidence pack", () => {
    expect(supplement.contract).toBe("DEVAM_SARTHI_GIVING_GUIDANCE_PILOT_SUPPLEMENT_V0_1");
  ...
  it("changes mode only after essentials and genuine surplus are explicit", async () => {
    expect(pair?.variants).toHaveLength(2);
    if (!pair) throw new Error("moral-04 pair missing");
