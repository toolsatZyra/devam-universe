...
describe("Sarthi consent-guidance pilot supplement", () => {
  const supplement = json("apps/web/evaluation/sarthi-consent-pilot-supplement-v0.1.json") as {
    contract: string;
    status: string;
  ...
  it("binds the frozen pilot, research manifests and source-bounded consent pack", () => {
    expect(supplement.contract).toBe("DEVAM_SARTHI_CONSENT_GUIDANCE_PILOT_SUPPLEMENT_V0_1");
  ...
  it("changes mode only after competent-adult refusal and pressure are explicit", async () => {
    expect(pair?.variants).toHaveLength(2);
    if (!pair) throw new Error("moral-02 pair missing");
