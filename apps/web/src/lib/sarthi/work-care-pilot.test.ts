...
describe("Sarthi work-care pilot supplement", () => {
  const supplement = json("apps/web/evaluation/sarthi-work-care-pilot-supplement-v0.1.json") as {
    contract: string;
    status: string;
  ...
  it("binds the frozen pilot, research manifests and source-bounded evidence pack", () => {
    expect(supplement.contract).toBe("DEVAM_SARTHI_WORK_CARE_PILOT_SUPPLEMENT_V0_1");
  ...
  it("changes mode only after the affected-party and safe-deferral facts are present", async () => {
    expect(pair?.variants).toHaveLength(2);
    if (!pair) throw new Error("guidance-04 pair missing");
