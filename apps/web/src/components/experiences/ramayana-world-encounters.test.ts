...
describe("Ramayana return-world encounters", () => {
  ...
  it("turns all fifty-five scenes in seven districts into resolvable in-world constellations", () => {
    expect(pack.districts.map((district) => district.momentIds.length)).toEqual([8, 8, 8, 8, 8, 8, 7]);
    expect(new Set(pack.districts.flatMap((district) => district.momentIds)).size).toBe(55);
    expect(Object.keys(pack.sceneNodeIds)).toHaveLength(55);
  ...
  it("indexes character and place encounters back into every playable story moment", () => {
    expect(pack.nodeMomentIds.rama).toEqual(["coronation-dawn", "rama-crosses-celebration", "rama-accepts-exile", "sita-chooses-road", "lakshmana-joins", "city-follows-car", "tamasa-night", "roads-beyond-kosala", "guha-night-watch", "ganga-crossing", "first-forest-night", "prayaga-to-yamuna", "chitrakoot-home", "empty-chariot-return", "bharata-rejects-boons", "guha-shows-first-night", "chitrakoot-hears-army", "brothers-meet-death-news", "family-asks-rama-home", "sandals-hold-kingdom", "nandigrama-trust", "chitrakoot-grows-unsafe", "sita-tells-her-beginning", "dandaka-receives-them", "viradha-breaks-the-road", "forest-asks-protection", "sita-questions-the-bow", "ten-years-become-map", "agastya-points-south", "jatayu-welcomes-panchavati", "surpanakha-breaks-quiet", "janasthana-falls", "golden-deer-separates-house", "mendicant-at-empty-cottage", "sita-carried-south", "jatayu-rises-sky-road", "leave-lanka", "sky-road", "bharadvaja-hermitage", "kingdom-returned"]);
  ...
  it("ships a bounded story pack instead of the global Atlas to the client", () => {
    ...
    expect(Buffer.byteLength(serialized)).toBeLessThan(335_000);
    expect(gzipSync(serialized).byteLength).toBeLessThan(55_000);
